...
module.exports = function (data) {
  ...
  var addBirthday = function (data, name, value) {
    return _addEvent(data, name, value, "born",
      "<http://dbpedia.org/ontology/birthPlace>", 
      ["<http://dbpedia.org/ontology/birthDate>",
       "<http://dbpedia.org/ontology/birthYear>"]);
  };
  var addDeathday = function (data, name, value) {
    return _addEvent(data, name, value, "died",
  ...
  var _addEvent = function (data, name, value, eventName, placeKey, dateKeys) {
    var placeId = getPlaceId(data, value[placeKey][0].value);

    if (placeId) {
      return addEvent.addEvent(name, value, eventName, placeId, dateKeys);
    }
  };
