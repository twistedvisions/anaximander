...
var eventInsert = _.template(
  "INSERT INTO event (name, place_id, start_date, end_date, link, type_id, importance_id) " +
  "VALUES ('<%= name %>', <%= place_id %>, " +
  "'<%= start_date %>', '<%= end_date %>', " +
  "'<%= link %>', <%= type_id %>, <%= importance_id %>) " +
  "RETURNING id"
);
var eventParticipantInsert = _.template(
  "INSERT INTO event_participant (thing_id, event_id, role_id, importance_id) " +
  "VALUES (<%= thing_id %>, <%= event_id %>, 15, 11)"
);
