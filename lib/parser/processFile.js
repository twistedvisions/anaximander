...
var extractLink = function (str) {
  try {
    ...
    winston.error("failed to extract link from", str);
    throw e;
  }
};
...
var processFile = function (file, totalLines, startReadingPc, finishReadingPc,
  ...
  if (totalLines > 1000) {
    winston.info("Reading lines ", humanize.numberFormat(linesToSkip, 0), 
      "through", humanize.numberFormat(linesToSkip + linesToTake, 0));
    winston.info("Processing places?", doPlaces);
    winston.info("Processing events?", doEvents);
  }
