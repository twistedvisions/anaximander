...
Search.prototype.handleSearchResults = function (res, results) {
  ...
  all = _.map(all, function (x) {
    ...
    if (x.points && x.dates) {
      var dates = _.map(x.dates.replace("{", "").replace("}", "").split(","), this.getDate, this);
      var importanceValues = x.importance_values;
      var points = JSON.parse(x.points).coordinates;
      var events = _.zip(points, dates, x.date_offset_seconds, x.event_names, importanceValues);

      x.points = _.map(events, function (event) {
        return {
          lat: event[0][1],
          lon: event[0][0],
          date: new Date(event[1]),
          date_offset_seconds: event[2],
          event_name: event[3],
          importance_value: event[4]
        };
      });
      delete x.dates;
      delete x.date_offset_seconds;
