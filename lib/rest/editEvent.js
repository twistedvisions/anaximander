...
EventEditor.prototype.getCalls = function (req/*, res, next*/) {
  ...
  return [
    ...
    //ensure participant role importances
    //handle added participants
    _.bind(this.addNewParticipants, this, this.params.newParticipants),
    //handle edited participants
    _.bind(this.updateExistingParticipants, this, this.params.editedParticipants)
...
EventEditor.prototype.addNewParticipants = function (participants) {
  return when.map(participants || [], _.bind(this.addParticipant, this));
};
...
EventEditor.prototype.updateExistingParticipants = function (participants) {
  return when.map(participants || [], _.bind(this.updateExistingParticipant, this));
};

EventEditor.prototype.updateExistingParticipant = function (participant) {
  return pipeline([
    _.bind(function () {
      if (participant.type && participant.type.id) {
        return db.runQueryInTransaction(
          this.tx, "update_participant_role",
          [participant.id, participant.thing.id, participant.type.id]
        );
      }
    }, this),
    _.bind(function () {
