...
EventSaver.prototype.ensureEventType = function (type) {
  if (type.id && (type.id > 0)) {
    return db.runQueryInTransaction(
      this.tx, "find_event_type_by_id", [type.id]
    ).then(
      function (result) {
        if (result.rows.length === 0) {
          winston.error("tried to find an event type that can't be found", type);
          throw {
            message: "no event type found with id: " + type.id
          };
        }
        return result.rows[0].id;
      },
    ...
    return db.runQueryInTransaction(
      this.tx, "save_event_type", [type.name]
    ).then(
      function (result) {
        if (result.rows === 0) {
          winston.error("tried to create an event type but failed", type);
          throw {
            message: "tried to create an event type but failed: " + JSON.stringify(type)
          };
        }
        return result.rows[0].id;
      },
...
EventSaver.prototype.createEvent = function (event) {
  ...
  if (validation) {
    ...
    return db.runQueryInTransaction(
      this.tx, "save_event", [event.name, event.type_id, event.place_id, event.start_date, event.end_date, event.link]
    ).then(
      function (result) {
        return result.rows[0].id;
      },
