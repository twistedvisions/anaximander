...
var groupEventByParticipant = function (eventArray) {
  var event = _.omit(eventArray[0], thingKeys);
  event.event_importance_value = _.max(eventArray, function (e) {
    return e.event_importance_value;
  }).event_importance_value;
  event.participants = _.map(eventArray, function (e) {
    return _.pick(e, thingKeys);
  });

  event.participants = _.chain(event.participants)
    .groupBy("thing_id")
    .map(function (list) {
      return _.max(list, function (item) {
        return item.importance_value;
      });
    })
    .sortBy(function (el) {
      return -el.importance_value;
    })
    .value();

  event.start_date = new Date(event.start_date.getTime() - event.start_date.getTimezoneOffset() * 60 * 1000);
  event.end_date = new Date(event.end_date.getTime() - event.end_date.getTimezoneOffset() * 60 * 1000);
...
var handleResult = function (res, result) {
  ...
  var events = _.map(byEvent, groupEventByParticipant);
