...
EventUtils.prototype.ensureParticipantTypes = function (participants, eventTypeId) {
  ...
  var newRoles = this.aggregateNewParticipantTypes(participants);
  var existingRoles = this.aggregateExistingParticipantTypes(participants);

  return sequence([
    _.bind(function () {
      return sequence(_.map(newRoles, _.bind(this.ensureNewParticipantTypes, this, eventTypeId)));
    }, this),
...
EventUtils.prototype.ensureParticipantImportances = function (participants) {
  ...
  var newImportances = this.aggregateNewParticipantImportances(participants);
  var existingImportances = this.aggregateExistingParticipantImportances(participants);
...
EventUtils.prototype.idExists = function (obj) {
  return obj.id && obj.id > 0;
};

EventUtils.prototype.aggregateNewParticipantTypes = function (participants) {
  var newRoles = {};
  _.each(participants, function (participant) {
    var type = participant.type;

    if (!this.idExists(type)) {
      if (type.name) {
...
EventUtils.prototype.ensureNewParticipantTypes = function (eventTypeId, typeList) {
...
EventUtils.prototype.aggregateExistingParticipantTypes = function (participants) {
  var existingRoles = {};
  _.each(participants, function (participant) {
    var type = participant.type;
    if (this.idExists(type)) {
...
EventUtils.prototype.aggregateNewParticipantImportances = function (participants) {
  var newImportances = {};
...
EventUtils.prototype.aggregateExistingParticipantImportances = function (participants) {
  ...
  db.runQueryInTransaction(
    ...
    function (result) {
      ...
      if (result.rowCount === 0) {
        ...
        if (params.start_date.get("year") <= 0) {
          params.start_date.subtract("year", 1);
        }
        if (params.end_date.get("year") <= 0) {
          params.end_date.subtract("year", 1);
        }
