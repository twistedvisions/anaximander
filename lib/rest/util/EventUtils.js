...
EventUtils.prototype.ensureParticipant = function (participant) {
  try {
    if (!participant.type) {
...
EventUtils.prototype.addParticipants = function (participants, eventId) {
...
EventUtils.prototype.ensureCreator = function () {
  var d = when.defer();
  if (this.creatorId) {
    d.resolve(this.creatorId);
  } else {
    db.runQueryInTransaction(
      this.tx, "save_creator", [this.userId]
    ).then(
