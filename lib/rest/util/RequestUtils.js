...
RequestUtils.prototype.ensure = function (
      obj, name,
      findQuery, findKeys,
      saveQuery, saveKeys,
      savePermissions
  ...
  if (obj.id && (obj.id > 0)) {
    ...
    if (!this.hasPermission(savePermissions, obj)) {
      d.reject(new Error("user lacks permission to create " + name));
    } else {
...
RequestUtils.prototype.hasPermission = function (permissions, obj) {
  if (_.isFunction(permissions)) {
    return permissions(obj);
  } else {
    return this.hasBasicPermission(permissions);
  }
};

RequestUtils.prototype.hasImportancePermission = function (obj) {
  return this.hasBasicPermission(["add-importance"]) ||
    (
      this.isNominalImportance(obj) &&
      this.hasBasicPermission(["add-type"])
    );
};

RequestUtils.prototype.isNominalImportance = function (obj) {
  return (obj.name.toLowerCase() === "nominal") &&
         (obj.value === 2);
};

RequestUtils.prototype.hasBasicPermission = function (permissions) {
